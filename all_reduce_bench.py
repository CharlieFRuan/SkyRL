...
python -u -m torch.distributed.run \
    ...
    --rdzv_backend c10d \
    all_reduce_bench.py > test_16.log 2>&1
...
if __name__ == "__main__":
    ...
    # NOTE: upstream hardcodes FI_PROVIDER=efa (AWS-only). This cluster uses
    # InfiniBand (NDR), so we don't force libfabric/EFA. NCCL net/HCA selection
    # is controlled via env (NCCL_IB_HCA, NCCL_SOCKET_IFNAME) at launch time.
    env_vars = {
        "NCCL_DEBUG": "INFO",
    }
