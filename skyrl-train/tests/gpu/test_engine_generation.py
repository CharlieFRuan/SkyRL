...
def test_inference_engines_generation_sglang():
    ...
    llm_client = init_ray_sglang_engines()
    # Batched generation.
...
def test_token_based_generation(backend):
    """Test generation using prompt_token_ids with both vLLM and SGLang backends."""
    ...
    # Initialize the appropriate engine based on backend
    if backend == "vllm":
        llm_client = init_ray_vllm_engines()
    elif backend == "sglang":
        llm_client = init_ray_sglang_engines()
