...
DATA_DIR="/workspace/data/searchR1"
...
uv run --isolated --frozen --extra vllm -m skyrl_train.entrypoints.main_base \
  ...
  trainer.run_name="charlie-0821-multiturn-non-TITO-skyrl-search_4turns_maxgeneratelen_500" \
  trainer.ckpt_interval=50 \
  trainer.hf_save_interval=100 \
  trainer.max_ckpts_to_keep=5 \
  trainer.resume_mode=latest \
  trainer.ckpt_path="/workspace/skyrl-search_4turns_maxgeneratelen_500" \
  trainer.eval_batch_size=256 \
  trainer.eval_before_train=false \
  generator.eval_sampling_params.temperature=0 \
  generator.eval_sampling_params.stop='["</search>", "</answer>"]' \
  trainer.export_path="/workspace/skyrl-search_4turns_maxgeneratelen_500/exports" \
